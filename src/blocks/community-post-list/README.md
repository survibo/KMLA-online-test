...
`CommunityPostList`는 특정 그룹 안의 게시글 목록 화면 레이아웃을 담당하는 block이다.

이 block은 그룹 헤더와 목록 컨테이너를 담당하고, 실제 게시글 내용은 기존 `CommunityPostCard`를 그대로 사용한다.

## Data Contract

입력 데이터는 아래 구조를 전제로 한다.

- 그룹 정보: `CommunityPostListGroup`
- 각 게시글: `CommunityPost`

현재 기준:

- 그룹 필드는 `groups` 스키마를 따라 `name`, `description`, `is_official`, `is_personal`, `created_at`를 유지한다.
- 그룹 안의 각 게시글은 `CommunityPostCard`로 렌더링한다.
- 이 block은 게시글 내부 형식을 다시 정의하지 않는다.
- 목록 mock도 `community-post-card`에서 쓰는 `CommunityPost` mock을 재사용한다.
...
- `mock.ts`: `community-post-card` mock을 재사용한 그룹 목록용 샘플 데이터
- `types.ts`: 그룹 목록용 타입
