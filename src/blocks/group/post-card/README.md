...
- 카드 본문은 기본 3줄까지만 노출하고, 실제로 넘칠 때만 `... 더보기`와 `접기`를 본문 클릭 토글로 제공한다.
- 댓글 drawer에서 답글을 기본 접기 상태로 둘지와 `답글 보기` 토글 방식
- 댓글 drawer는 먼저 열리고, 댓글 목록은 상단 몇 개를 짧게 stagger reveal 해서 댓글 수가 많아도 초기 체감이 갑자기 무겁게 보이지 않도록 한다.
...
- 카드 자체 memo는 콜백 핸들러 stale 위험을 피하기 위해 기본 shallow 비교를 사용하고, 커스텀 comparator로 일부 props를 누락하지 않는다.
