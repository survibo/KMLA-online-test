...
이 문서는 프로젝트 전반에 공통으로 적용되는 작업 규칙을 정의한다.
...
- block 작업에서 빠뜨리기 쉬운 점검 항목을 줄인다.

이 문서는 공통 규칙만 다룬다.
특정 기능이나 화면에만 적용되는 규칙은 넣지 않는다.
...
단, 특정 block에만 적용되는 규칙은 해당 block의 `README.md`를 우선한다.

## Session Start Rule

새 작업이나 요청이 들어오면 먼저 `AGENTS.md`를 기준으로 판단하고 진행한다.
중요한 전역 규칙은 이 문서를 먼저 다시 확인한다.
...
- 화면만 고치고 끝내지 않는다.
- block만 고치고 mock이나 문서를 남겨두지 않는다.
...
- 한글이 포함된 파일은 가능하면 UTF-8 기준으로 읽는다.

권장 방식:

- `Get-Content -Encoding utf8 <path>`
- PowerShell 세션의 입출력 인코딩을 UTF-8로 맞춘다.
- IDE에서 정상 표시되는 문자열과 터미널 표시가 다르면 터미널 출력 대신 파일 원문을 기준으로 판단한다.

주의:

- 터미널에서만 mojibake처럼 보여도 실제 파일은 정상 UTF-8일 수 있다.
- 이 경우 바로 "문자열 복구"를 하지 말고 먼저 출력 경로의 인코딩 문제인지 확인한다.
...
- 화면 전용 계산값은 raw data와 분리한다.

보완 원칙:

- `dbdiagram`으로 표현하기 어려운 제약은 `scheme.md`의 `Note`에 남긴다.
- 실제 구현에서는 그 제약을 별도로 보완한다.
...
- 화면 로직이 그 mock만으로 실제 흐름을 검증할 수 있어야 한다.

핵심 원칙:

- mock도 데이터 계층을 나눠서 본다.
...
- 이 경우 도메인 루트에 schema-aligned raw mock data를 둔다.
- 예: `src/blocks/<domain>/mock.ts`
- 도메인 루트 `mock.ts`에는 users, rooms, members, messages 같은 raw data와 이를 조회하거나 계산하는 helper를 둔다.
- 도메인 루트 `mock.ts`와 block 내부 `mock.ts`는 파일 이름만 같고 역할은 다르다.
- raw mock은 가능한 한 실제 DB/API shape와 비슷하게 유지한다.
- raw mock에는 가능하면 join 결과나 화면 전용 계산값을 미리 넣지 않는다.
- 다만 프론트 실험에 꼭 필요하면 제한적으로 join 결과를 둘 수 있다.
- unread 여부, latest item, display text, 정렬 결과 같은 파생값은 raw mock이 아니라 projection에서 계산한다.
...
- scenario 파일에서 raw와 projection 둘 다 보관할지는 선택할 수 있다.
- projection 계산 경로는 한 곳으로 유지한다.
- scenario마다 직접 계산식을 흩뿌리지 말고 기존 projection helper를 다시 호출한다.
- edge case는 base mock을 오염시키지 말고 scenario에서 표현한다.
- block이 사실상 단독으로만 쓰이고 아직 도메인 공용 데이터가 필요 없을 때만 block 내부 base mock으로 시작해도 된다.
- 같은 도메인 안에서 두 번째 block부터는 도메인 루트 raw mock 구조를 우선 검토한다.
...
- block 안으로 들어간 뒤에는 preview 전용 wrapper를 최소화하고 실제 화면과 최대한 비슷하게 보여준다.
- block route에 들어간 뒤에는 "테스트 UI"보다 "실제 화면 재현"을 우선한다.
...
도메인 단위 block 구조를 쓰는 경우의 권장 기본 구조:
...
- mock을 자주 바꿔가며 실험하는 block이면 두는 쪽을 권장한다.
...
- 도메인 안의 개별 block도 각자 `index.tsx`, `mock.ts`, `mock.scenarios.ts`, `types.ts`, `README.md`를 기본 단위로 유지한다.
...
- 파일 하나에서만 쓰는 단순 레이아웃, flex/grid 배치, 국소 spacing은 JSX/Tailwind에 남겨도 된다.
- 공용 CSS는 기본 선택지가 아니며 꼭 필요할 때만 사용한다.
- 사용자가 공용 CSS 사용을 명시적으로 요청했다면 그 방향을 따른다.
...
- 타입이 현재 props와 data shape를 설명하는가
...
- 시나리오가 추가되거나 이름 또는 구조가 바뀌면 preview 진입점도 함께 점검한다.
...
세부 정책은 작업 중 관련 위치의 md 파일에 남긴다.

새 규칙을 추가할 때는 먼저 아래를 판단한다.

- 이 규칙이 프로젝트 전반에 공통인가
- 특정 도메인이나 특정 block에만 해당하는가
